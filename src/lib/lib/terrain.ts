...
export type TerrainBlock = "grass" | "stone" | "coal_ore" | "wood" | "leaves";
...
export function generateTerrain(seed: number): Map<string, TerrainBlock> {
  ...
  const coalAt = (x: number, y: number, z: number) => {
    // Sparse deterministic vein centres, each spreading into a compact cluster.
    ...
    for (let dx = -1; dx <= 1; dx += 1) {
      for (let dy = -1; dy <= 1; dy += 1) {
        for (let dz = -1; dz <= 1; dz += 1) {
          ...
          const cy = cellY + dy;
          const cz = cellZ + dz;
          const chance = hash2(cx * 19.1 + cy * 43.7 + seed * 7.3, cz * 31.9 - cy * 11.2);
          if (chance < 0.84) continue;
          const centerX = cx * 4 + hash2(cx + seed, cz - cy) * 4;
          const centerY = cy * 3 + hash2(cy + seed * 2, cx + cz) * 3;
          const centerZ = cz * 4 + hash2(cz - seed, cx - cy) * 4;
          const distance =
            Math.pow((x + 0.5 - centerX) / 2.3, 2) +
            Math.pow((y + 0.5 - centerY) / 1.7, 2) +
            Math.pow((z + 0.5 - centerZ) / 2.3, 2);
  ...
  for (let x = -WORLD_RADIUS; x <= WORLD_RADIUS; x += 1) {
    for (let z = -WORLD_RADIUS; z <= WORLD_RADIUS; z += 1) {
      ...
      for (let y = 0; y < top; y += 1) {
        solid.set(blockKey(x, y, z), y >= top - soil ? "grass" : coalAt(x, y, z) ? "coal_ore" : "stone");
      }
    }
  }
